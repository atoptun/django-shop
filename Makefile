...
doc-start-prod: ## Start the production environment
	docker compose -p $(DOCKER_PROD_PROJECT_NAME) --env-file .env -f compose.yml -f compose.prod.yml up -d --build

doc-stop-prod: ## Stop the production environment
	docker compose -p $(DOCKER_PROD_PROJECT_NAME) --env-file .env -f compose.yml -f compose.prod.yml down
...
db-migrate: ## Generate a local migration script (Usage: make db-migrate m="migration description")
	...
	POSTGRES_HOST=$(LOCAL_DB_HOST) POSTGRES_PORT=$(LOCAL_DB_PORT) uv run alembic revision --autogenerate -m "$(m)"
...
db-history: ## List the full migration history from the Docker container context
	POSTGRES_HOST=$(LOCAL_DB_HOST) POSTGRES_PORT=$(LOCAL_DB_PORT) uv run alembic history -v
...
doc-migrate: ## Generate a migration script INSIDE Docker container (Usage: make doc-migrate m="description")
	...
	docker compose -p $(DOCKER_DEV_PROJECT_NAME) exec api uv run alembic revision --autogenerate -m "$(m)"

doc-upgrade: ## Apply all pending migrations INSIDE Docker container
	docker compose -p $(DOCKER_DEV_PROJECT_NAME) exec api uv run alembic upgrade head

doc-current: ## Display the current migration revision of the Docker database
	docker compose -p $(DOCKER_DEV_PROJECT_NAME) exec api uv run alembic current

doc-history: ## List the full migration history from the Docker container context
	docker compose -p $(DOCKER_DEV_PROJECT_NAME) exec api uv run alembic history -v
